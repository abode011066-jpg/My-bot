* (bet / 9.0) # الرهان مقسم على 9 خطوط
...
def evaluate_grid(grid, bet):
    ...
    for line_idx, line in enumerate(paylines):
        ...
        # 🎯 جدول المضاعفات المحدث بدقة حسب طلبك
        if first_sym == '7':
...
# --- توليد الشبكة مع منع السبعات وحصرها في الأرباح العالية جداً ---
def generate_controlled_grid(tier, bet, forced_jars=0, max_win_cap=None):
    jar_mults = [1, 2, 3, 5]
    ...
    for _ in range(300):
        ...
        # 🎯 تحديد حزم الرموز: رمز '7' محظور تماماً ويظهر فقط بنسبة ضئيلة جداً في نمط win20 و win50
        if tier in ["win20", "win50"]:
            symbols_pool = ['🍋', '🍍', '🍊', '🍒', '🍉', '🔔', '🍇', '⭐', '$', '7']
            weights = [15, 15, 15, 15, 12, 12, 12, 3, 3, 1]  # السبعات نادرة جداً جداً
        else:
            symbols_pool = ['🍋', '🍍', '🍊', '🍒', '🍉', '🔔', '🍇', '⭐', '$']
